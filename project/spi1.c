...
#define DEVICE_NAME "ili9225_rani"
                         ...
                         loff_t *off)
{
    ...
    snprintf(lcd_buffer,sizeof(lcd_buffer),"%s",kbuf);
...
static ssize_t ili_read(struct file *file,
                        char __user *buf,
                        size_t len,
                        loff_t *off)
{
    int l = strlen(lcd_buffer);

    if(l == 0)
        return 0;

    if(copy_to_user(buf,lcd_buffer,l))
        return -EFAULT;
...
static const struct of_device_id ili9225_dt_ids[] = {
    { .compatible = "ilitek,ili9225_rani" },
    { }
};
