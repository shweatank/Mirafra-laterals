...
#define TIMER_INTERVAL_NS	500000000
...
{
	printk(KERN_INFO "Process going to sleep\n");

	wait_event_interruptible(wait_queue, condition != 0);

	condition = 0;

	printk(KERN_INFO "Process woke up\n");
...
static int waitq_close(struct inode *inode, struct file *file)
{
	return 0;
}
...
static int __init waitq_driver_init(void)
{
	...
	printk(KERN_INFO "Wait queue drive loaded\n");
...
MODULE_DESCRIPTION("Linux Wait Queue Demo");
