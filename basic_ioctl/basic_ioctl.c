...
#define IOCTL_SET_VALUSE _IOWR(IOCTL_MAGIC,1,int)
						...
						unsigned long arg)
{
	...
	switch(cmd)
	{
		case IOCTL_SET_VALUSE :
			/*copy data from user*/
			if(copy_from_user(&user_value, (int __user *)arg, sizeof(int)))
				return -EFAULT;
			pr_info("krnal: received %d from user\n",user_value);

			/*kernal modify the data*/
...
static struct file_operations foos = {
		.owner          = THIS_MODULE,
     	.unlocked_ioctl = basic_ioctl,
};	
static int __init basic_init(void)
{
	major = register_chrdev(0, DEVICE_NAME, &foos);
